...
* Spending more time pseudo-coding in the beginning will help prevent long and complex refactors later down the process
* Significantly reducing image/content file size is increasingly necessary. Most likely implementing dynamic image loading, as opposed to loading all image on page load. The initial consideration was that compressing images down to JPEG medium and loading all of them at page load would stop any latency between clicking a button and dynamically inserting the image. This caused a huge issue with network payload on our dev tools audit, and the tradeoff in latency was definitely not worth the cost. 
